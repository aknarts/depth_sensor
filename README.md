...
Publishes water depth (cm) and board temperature via Zigbee (Home Assistant through Zigbee2MQTT).  
An onboard LED is exposed via Zigbee for basic status/identify.

## Features
- Depth reporting from 4–20 mA pressure sensor (Analog Output cluster presentValue in cm).
...
- Depth is reported in centimeters (cm) to Zigbee for compatibility with the previous implementation.
...
## Zigbee
- Endpoint: 1
- Clusters (server): Basic, Identify, Analog Output (depth in cm), Temperature Measurement, On/Off, Color Control, Level, Scenes, Groups
- Reporting:
